...
/*   Updated: 2024/10/23 14:53:39 by araveala         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/*int	draw_section(t_data *data, double wall_h, double wall_i, int x)
{
	...
	{
		if (wall_i > 0 && wall_i < HEIGHT)
		{
			if (x >= 0 && x < WIDTH && data->ray_len[x] > 0)
				mlx_put_pixel(data->im_ray, x * (WIDTH / RAY_MAX), (int)wall_i, rgb(data, img_x, img_y));
		}
